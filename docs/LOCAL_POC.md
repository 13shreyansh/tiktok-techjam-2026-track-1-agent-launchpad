...
Only the selected model API is remote. Volcengine Ark remains the default; an
authorized OpenAI-compatible Responses endpoint may be selected through the
documented `MODEL_*` variables.
...
- An authorized Responses-compatible API key and model/endpoint ID

```bash
ARK_API_KEY=your-ark-api-key ARK_MODEL=ep-your-endpoint-id npm run poc
```

Or use `MODEL_API_KEY`, `MODEL_ID`, and optionally `MODEL_BASE_URL` for the
