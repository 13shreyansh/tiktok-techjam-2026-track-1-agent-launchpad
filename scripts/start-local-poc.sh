...
if [[ ( "$generic_model_requested" == true \
      && ( -z "${MODEL_API_KEY:-}" || -z "${MODEL_ID:-}" ) ) \
  || ( "$generic_model_requested" != true \
      && ( -z "${ARK_API_KEY:-}" || -z "${ARK_MODEL:-}" ) ) ]]; then
  log "MODEL_API_KEY and MODEL_ID, or ARK_API_KEY and ARK_MODEL, are required."
  log "A partial MODEL_* configuration does not fall back to Ark."
  log "Example: MODEL_API_KEY=key MODEL_ID=model MODEL_BASE_URL=https://provider.example/v1 ./scripts/start-local-poc.sh"
  exit 2
fi
