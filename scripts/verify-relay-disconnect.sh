...
nats_port="${RELAY_DISCONNECT_VERIFY_NATS_PORT:-4344}"
proof_root="$(mktemp -d "${TMPDIR:-/tmp}/relay-disconnect-proof.XXXXXX")"
...
cleanup() {
  ...
  case "$proof_root" in
    "${TMPDIR:-/tmp}"/relay-disconnect-proof.*) rm -rf "$proof_root" ;;
...
for _ in {1..50}; do
  if kill -0 "$nats_pid" 2>/dev/null && node -e "const net=require('node:net');const socket=net.connect($nats_port,'127.0.0.1',()=>{socket.end();process.exit(0)});socket.on('error',()=>process.exit(1));setTimeout(()=>process.exit(1),100).unref()"; then
    node scripts/verify-nats-disconnect.mjs
    exit 0
  fi
  sleep 0.1
done
