...
nats_port="${RELAY_CANCEL_VERIFY_NATS_PORT:-4342}"
proof_root="$(mktemp -d "${TMPDIR:-/tmp}/relay-cancel-proof.XXXXXX")"
...
cleanup() {
  ...
  case "$proof_root" in
    "${TMPDIR:-/tmp}"/relay-cancel-proof.*) rm -rf "$proof_root" ;;
...
start_nats
before_restart="$(node scripts/verify-nats-cancellation.mjs write)"
printf '%s\n' "$before_restart"
export RELAY_CANCEL_SESSION_ID="$(printf '%s' "$before_restart" | \
...
start_nats
node scripts/verify-nats-cancellation.mjs read
