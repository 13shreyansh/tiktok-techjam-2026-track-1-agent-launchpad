...
nats_port="${RELAY_RESTART_VERIFY_NATS_PORT:-4340}"
proof_root="$(mktemp -d "${TMPDIR:-/tmp}/relay-restart-proof.XXXXXX")"
...
cleanup() {
  ...
  case "$proof_root" in
    "${TMPDIR:-/tmp}"/relay-restart-proof.*) rm -rf "$proof_root" ;;
...
start_nats
before_restart="$(node scripts/verify-nats-coordinator.mjs write)"
printf '%s\n' "$before_restart"
export RELAY_RESTART_SESSION_ID="$(printf '%s' "$before_restart" | \
  node -e 'let text="";process.stdin.on("data",chunk=>text+=chunk).on("end",()=>process.stdout.write(JSON.parse(text).sessionId))')"
kill "$nats_pid"
wait "$nats_pid" 2>/dev/null || true
nats_pid=""

start_nats
node scripts/verify-nats-coordinator.mjs read
